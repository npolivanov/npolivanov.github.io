...
function drawCell(cell) {
  ...
  switch (cell.value){
    ...
    case 8 : ctx.fillStyle = '#91ABFF'; break;
    case 16 : ctx.fillStyle = '#00FF04'; break;
...
document.onmousedown = function (event)
	{
		...
		{
			console.log(firstX  + ' ' + event.pageX);
			
			if( Math.abs(firstX - event.pageX) >   Math.abs(firstY - event.pageY))
			{
				if( firstX >  event.pageX)
				{
					moveLeft();
				}
				else
...
pause = () =>
{
	...
	if( cnv.style.filter != 'blur(7px)' )
	{
		cnv.style.filter = 'blur(7px)';
		document.querySelector('.pause').style.display = "inline";
	}else
	{
		cnv.style.filter = 'blur(0px)';
